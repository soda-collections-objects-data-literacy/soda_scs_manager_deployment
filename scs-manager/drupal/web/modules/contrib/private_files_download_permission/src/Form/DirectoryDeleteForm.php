...
class DirectoryDeleteForm extends EntityConfirmFormBase {
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    try {
      ...
      \Drupal::messenger()->addStatus($this->t('The directory %path was deleted successfully.', ['%path' => $pfdp_directory->path]));
    }
    catch (EntityStorageException $exception) {
      $this->logger('pfdp')->error('The directory %path was not deleted.', ['%path' => $pfdp_directory->path]);
      \Drupal::messenger()->addError($this->t('The directory %path was not deleted.', ['%path' => $pfdp_directory->path]));
    }
